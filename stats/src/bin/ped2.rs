...
fn main() {
    ...
    let pts = bench::<YokedMetaAutocompleter>(1e2 as usize, 2);
    ...
    let trace = Scatter::new(vx, vy)
        .mode(Mode::Markers)
        .marker(Marker::new().size(3).color(NamedColor::HotPink))
        .name("PED<=2");
    plot.add_trace(trace);

    let pts = bench::<YokedMetaAutocompleter>(1e2 as usize, 3);
    let mut vx = vec![];
    let mut vy = vec![];
    for p in pts {
        vx.push(p.query_len);
    ...
    let trace = Scatter::new(vx, vy)
        .mode(Mode::Markers)
        .marker(Marker::new().size(3).color(NamedColor::DarkOrange))
        .name("PED<=3");
    plot.add_trace(trace);
